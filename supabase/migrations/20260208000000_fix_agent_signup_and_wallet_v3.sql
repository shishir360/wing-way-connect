...
-- 0. SELF-HEALING: ENSURE app_role TYPE EXISTS
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN
        CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user', 'agent');
    ELSE
        -- If it exists, ensure 'agent' is in it
...
WITH CHECK (
  auth.uid() = user_id 
  AND role IN ('agent'::public.app_role, 'user'::public.app_role) -- Use the explicit type
);
...
-- 5. TRUNCATE WALLETS AND RE-POPULATE (Optional, but safe)
-- We truncate to remove orphans if user ID changed (unlikely)
